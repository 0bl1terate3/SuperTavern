    ...
    return function (req, res, next) {
        ...
        if (!whitelist.some(x => ipMatching.matches(clientIp, ipMatching.getMatch(x)))
            ...
            if (!noLogPaths.includes(req.path)) {
                console.warn(
                    color.red(
                        `Blocked connection from ${ipDetails}; User Agent: ${userAgent}\n\tTo allow this connection, add its IP address to the whitelist or disable whitelist mode by editing config.yaml in the root directory of your SuperTavern installation.\n`,
                    ),
                );
            }
