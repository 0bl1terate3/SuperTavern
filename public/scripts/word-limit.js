import { chat_metadata, generateQuietPrompt, saveChatDebounced, characters, this_chid } from '../script.js';
 ...
 * @returns {Object|null} Word limit settings or null if not enabled
 */
function getCharacterWordLimitSettings() {
    if (typeof characters === 'undefined' || typeof this_chid === 'undefined' || this_chid === undefined) {
        return null;
    }
    ...
    return {
        ...
        min: character.data.word_limit_min || 10,
        max: character.data.word_limit_max || 100,
        strictMode: character.data.word_limit_strict_mode || false,
        autoTrim: character.data.word_limit_auto_trim !== false,
        style: character.data.word_limit_style || 'natural',
        instructions: character.data.word_limit_instructions || ''
    };
}

/**
 * Apply character-specific word limit settings to a response
 * @param {string} response - The response text to process
 * @returns {string} Processed response
 */
function applyCharacterWordLimit(response) {
    const settings = getCharacterWordLimitSettings();
    if (!settings || !settings.enabled) {
        return response;
    }

    const words = splitWords(response);
    ...
    if (wordCount < settings.min) {
        if (settings.strictMode) {
            // In strict mode, we might want to regenerate or add padding
    ...
    if (wordCount > settings.max) {
        if (settings.autoTrim) {
            ...
            // In strict mode, we might want to regenerate
...
export { WORD_LIMIT_METADATA_KEY, getCharacterWordLimitSettings, applyCharacterWordLimit };
