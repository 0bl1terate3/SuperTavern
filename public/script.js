...
jQuery(async function () {
    ...
    // Function to get the last AI message from the chat
    function getLastAiMessage() {
        if (!Array.isArray(chat) || chat.length === 0) {
            return null;
        }

        // Look for the last non-user, non-system message
        for (let i = chat.length - 1; i >= 0; i--) {
            const message = chat[i];
            if (!message.is_user && !message.is_system && message.mes && message.mes.trim()) {
    ...
    // Append last AI response to character description - using event delegation for dynamically created elements
    $(document).on('click', '#append_last_response_button', async function () {
        const button = $(this);
        ...
        try {
            ...
            if (!lastAiMessage) {
                toastr.warning('No AI response found to append');
                return;
            }
            ...
            // Show loading state
            button.attr('class', 'menu_button fa-solid fa-spinner fa-spin');
            button.prop('disabled', true);

            // Call the backend API to append the content
            const response = await fetch('/api/characters/append-description', {
                method: 'POST',
                headers: getRequestHeaders(),
                body: JSON.stringify({
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Failed to append description');
            }

            const result = await response.json();

            // Update the UI with the new description
            $('#description_textarea').val(result.newDescription);

            // Update the character data in memory
            currentCharacter.description = result.newDescription;
            if (currentCharacter.data) {
                currentCharacter.data.description = result.newDescription;
            }

            toastr.success('AI response added to character description');

            // Emit character edited event to update other parts of the UI
            await eventSource.emit(event_types.CHARACTER_EDITED, {
                detail: { id: this_chid, character: currentCharacter }
            });

        } catch (error) {
            console.error('Error appending AI response to description:', error);
            toastr.error(`Failed to append AI response: ${error.message}`);
        } finally {
            // Restore button state - allow multiple uses
