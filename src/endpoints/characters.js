...
/**
 * Handle a POST request to append content to a character's description.
 *
 * This function reads the character data from a file, appends the provided content
 * to the description field, and writes the updated data back to the file.
 *
 * @param {Object} request - The HTTP request object.
 * @param {Object} response - The HTTP response object.
 * @returns {void}
 */
router.post('/append-description', validateAvatarUrlMiddleware, async function (request, response) {
    console.debug('Append description request:', request.body);
    ...
    if (!request.body.avatar_url || !request.body.content) {
        console.warn('Error: missing required fields (avatar_url, content)');
    ...
    try {
        ...
        const currentDescription = char.description || '';

        // Append new content with a newline separator
        const separator = currentDescription.trim() ? '\n' : '';
        const newDescription = currentDescription + separator + request.body.content;

        // Update both V1 and V2 description fields
        char.description = newDescription;
        if (char.data) {
            char.data.description = newDescription;
        }
        ...
        console.error('An error occurred while appending to character description:', err);
        return response.status(500).send({
            message: 'Unexpected error while updating character description.',
            error: err.toString()
        });
    }
});
