...
function exportSharedMemorySnapshot() {
    ...
    const payload = {
        ...
        projectKey,
        entries: entries.map(({ id, author, role, text, timestamp, direction, type }) => ({
            id,
            author,
            role,
            text,
    ...
    download(JSON.stringify(payload, null, 2), 'supertavern-shared-memory.json', 'application/json');
    window.toastr?.success('Shared memory exported.', 'SuperTavern');
}
...
function clearSharedMemory() {
    ...
    render();
    schedulePersist();
    window.toastr?.success('Shared memory cleared.', 'SuperTavern');
}

function getMemoryKey() {
    const chatId = dependencies.getCurrentChatId?.() || 'default';
    const character = dependencies.getActiveCharacter?.() || 'default';
    return `${chatId}-${character}`;
}

function getRoomKey() {
    const chatId = dependencies.getCurrentChatId?.() || 'default';
    const character = dependencies.getActiveCharacter?.() || 'default';
    const group = dependencies.getActiveGroup?.() || 'default';
    return `${chatId}-${character}-${group}`;
}
